<template>
  ...
  >
    ...
    />
    <div v-if="fileListRef && fileListRef.length > 0">
      <FrameSelector
        :thumbUrl="fileListRef[0]['thumbUrl']"
        @update:border-width="handleUpdateBorderWidth"
...
<script lang="ts" setup>
  ...
  // utils
  import { checkImgType, getBase64WithFile, checkVideoType, getVideoThumbnail } from '../helper';
  ...
  import FrameSelector from '@/components/frameselector/FrameSelector.vue';
  ...
  const emit = defineEmits(['change', 'register', 'delete', 'taskId', 'borderWidth']);
  ...
  function beforeUpload(file: File) {
    ...
    } else if (checkVideoType(file)) {
      getVideoThumbnail(file).then(({ result: thumbUrl }) => {
        fileListRef.value = [
          ...unref(fileListRef),
          {
            thumbUrl,
  ...
  async function uploadApiByItem(item: FileItem) {
    try {
      ...
      formData.append('margin', Math.round(borderWidth.value)); // 添加边框宽度
